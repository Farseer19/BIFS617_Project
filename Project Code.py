...
def find_orfs_in_frame(seq, min_len, reverse):
    ...
    orfs = []
    # Checks if this is a reverse complement call and changes the sequence to the reverse complement
    if reverse:
...
def orfs_from_input(min_len):
    ...
    for entry in output_dict:
        for orf in output_dict[entry]:
            # Prints out Frame, Position and Length information
            print(entry, "|FRAME=",orf[0],"POS=",orf[1],"LEN=",len(orf[2]))
            # Creates string of space separated codons from the sequence
